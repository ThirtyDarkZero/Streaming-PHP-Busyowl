...
window.bof_input = {
  ...
  hook: function(){
    ...
    /*if ( window.user.logged ){
      var iH = CryptoJS.MD5( $(document).find("#l"+"ogo im"+"g").attr("src").substr(22) ).toString();
      if ( iH != "a64cc2a3b1fc8d958a578f05c5c13be0" && iH != "60690a8a62d16c2a2aa178b1c30920a5" && iH != "d60b4b425d410f154a0de908427566c1" )
  ...
  hook_daterangepicker: function(){
    ...
    if ( $('.bof_time_range').length ){
      ...
      var ranges = $('.bof_time_range');
      for ( var i=0; i<ranges.length; i++ ){

        var range = $(ranges[i]);
        var rangeVal = range.val();
        var rangeStart = rangeVal.includes(" - ") ? range.val().split( " - " )[0] : undefined;
        var rangeEnd = rangeVal.includes(" - ") ? range.val().split( " - " )[1] : undefined;

        range.daterangepicker({
          "showDropdowns": true,
          "timePicker": true,
          "timePicker24Hour": true,
          "startDate": moment( rangeStart ),
    ...
    if ( $('.bof_input[type=datetime]').length ){

      var pickers = $('.bof_input[type=datetime]');
      ...
      $('.bof_input[type=datetime]').on('apply.daterangepicker', function(ev, picker) {
  ...
  listen: function(){
    ...
    $(document).on( "click", ".file_input_wrapper .mask", window.bof_input.call )
    ...
    $(document).on( "click", ".file_input_wrapper .preview_buttons .edit_cap", function(){

      var p = $(this).parents(".file_input_wrapper");
      var s = $(this).parents(".setting_wrapper");
      var i = p.find("input.cc").val();
      window.bof_modal.create({
        class: "mut import img_cap_modal",
        title: "Image alt/caption",
        inputs: {
          caption: {
            title: "Enter image's alt text",
            input: {
              type: "textarea",
              name: "caption",
              value: i
        ...
        buttons: [
          [ "btn-primary", "Save", "$(document).find(\"#"+s.attr("ID")+" input.cc\").val( $(document).find(\".img_cap_modal textarea\").val() ); window.bof_modal.close();" ]
        ],
      });
    ...
    $(document).on( "click", ".bof_dropdown.object_dropdown .search_input_wrapper .btn-primary", function(){
      ...
      window.becli.exe({
        ...
        callBack: function( sta, data ){
          ...
          window.ui.theme.part( "parts/object_search_result", { target: false } ).done(function(htmlData){
    ...
    $(document).on( "click", ".select_i_c_wrapper .select_i_c", function(e){

      var value = $(this).data("value");
      $(this).parents(".select_i_c_wrapper").find("input._value").val( value );
      $(this).parents(".select_i_c_wrapper").find(".thisOne").removeClass("thisOne")
  ...
  call: function(){
    ...
    var handler = $(this).parents(".file_input_wrapper");
    var setting = {};
    setting["type"] = handler.data( "type" );
    setting["object_type"] = handler.data( "object-type" );
    setting["accept"] = handler.data( "accept" ); setting["chunk"] = handler.data( "chunk" ) == "yes";
    setting["chunk_size"] = handler.data( "chunk-size" );
    handler.addClass( ID );

    var endpoint = "bofInput/file/";

    if ( handler.data("endpoint") )
    endpoint = handler.data("endpoint");

    setting["endpoint"] = endpoint;

    $("body").append("<form style='display:none' class='dropzone_form' ID='"+ID+"' data-endpoint='"+endpoint+"' data-hasFile='yes' data-type='file' data-callback='window.bof_input.callback' data-callback_param='"+ID+"'>\
    ...
    .done(function(){
      ...
      var args = {
        url: window.config.endpoint_address + setting["endpoint"] + "?type=" + setting["type"] + "&object_type=" + setting["object_type"],
        paramName: "$file",
        acceptedFiles: setting["accept"],
        createImageThumbnails: false,
        init: function(){
          window.bof_input.dropzone_init( this, ID )
        },
        maxFiles: 1,
        // forceChunking: true
      }
  ...
  dropzone_init: function( myDropzone, ID ){
    ...
    myDropzone.on("error", function(a1,a2,a3) {
      console.log( "error" );
      console.log( a1 );
      console.log( a2 );
      console.log( a3 );
  ...
  dropzone_progress: function( event, myDropzone, ID ){
    ...
    for ( var i=0; i<myDropzone.files.length; i++ ){
      ...
      _report.total_progress += __file.status == "error" ? 0 : __file.upload.progress;
      _report.total_size_uped += __file.status == "error" ? 0 : __file.upload.total;

      if ( __file.status == "error" )
      _report.total_fail++;

      else if ( __file.upload.progress == 100 && __file.status == "success" )
      _report.total_done++;
    ...
    if ( _report.total_fail + _report.total_done == _report.total_files && _report.total_files >= 1 && _report.total_active == 0 ){
      ...
      console.log( __done, __response, ID, myDropzone );
  ...
  callback: function( sta, data, ID ){
    ...
    console.log( sta, data, ID );

    if ( sta ? data.messages[0] == "done" : false ){

      $(document).find(".file_input_wrapper."+ID).addClass("_done").find(".mask_text").text("done");
      $(document).find(".file_input_wrapper."+ID + " .ci").val( data.file_id );
      $(document).find(".file_input_wrapper."+ID + " .cp").val( data.file_pass );

      $(document).find(".file_input_wrapper."+ID).addClass( "type_" + data.type )
    ...
    $(document).trigger( "bof_input_callback", {
      sta: sta,
      data: data,
      ID: ID
    } );
  ...
  object_selector_ini: function( defPages, objects ){
    ...
    for ( var i=0; i<Object.keys(objects).length; i++ ){
      ...
      var object_name = Object.keys(objects)[i];
      var object_data = objects[ object_name ];
      objects_option.push( [ object_name, object_data["label"] ] );
      object_data["selector"]["display_on"] = {
        object_type: [ "equal", object_name ]
      }
      object_data["selector"]["group"] = "b";
      objects_inputs[ "object_" + object_name ] = object_data["selector"];
    ...
    var inputs = {
      object_type: {
         title: "Object type",
         tip: "What kind of object you wish to make link to? For example if you wish to make a link to a page, select 'page'. To link a blog-post, select `Blog-Post`",
         input: {
           name: "object_type",
           type: "select",
           options: objects_option
         },
         group: "b"
       }
    };
    ...
    var $html = "";
    $html = $html + "<div class='modal_hover close_opm'></div>";
    $html = $html + "<div class='modal_wrapper opm_wrapper'><div class='modal opm'>";
    $html = $html + "<div class='title'>Find a link<span class='material-icons-outlined close_opm'>close</span></div>";

    window.bof_modal.render_inputs( $.extend( inputs, objects_inputs ) ).done( function( inputs_rendered ){

      if ( inputs_rendered )
      $html = $html + "<div class='inputs'>"+ inputs_rendered +"</div>";

      $html = $html + "<div class='buttons'>";
      $html = $html + "<div class='button'><a class='btn btn-primary execute_opm'>Get link</a></div>";
      $html = $html + "<div class='button'><a class='btn btn-secondary close_opm'>Back</a></div>";
      $html = $html + "</div>";
      ...
      window.bof_input.exe_display_rules( $.extend( inputs, objects_inputs ) );

      $(document).on( "change", ".modal.opm .bof_input", function(e){
        window.bof_input.exe_display_rules( $.extend( inputs, objects_inputs ) );
      });

    } );

    $(document).on( "click", ".close_opm", function(e){
      window.bof_input.object_selector_off();
    } );
    $(document).on( "click", ".modal.opm .buttons .btn-primary", function(e){

      var Type = $(document).find(".modal.opm select[name=object_type] option:selected").val();
      var Id = $(document).find(".modal.opm .setting_wrapper:not(.hideByRules) input._ids").val();
      if ( Type == "_defs" ){
        var Link = $(document).find(".modal.opm .setting_wrapper:not(.hideByRules) select[name=_defs] option:checked").val();
        $(document).find(".modal input[name=href]").val( Link );
        window.bof_input.object_selector_off();
        return;
      }
      ...
      window.becli.exe({
        ...
        callBack: function( sta, data ){
          ...
          if ( sta ){
            if ( data.link ){
              $(document).find(".modal input[name=href]").val( data.link );
            }
          }
        }
      })
  ...
  object_selector_off: function(){
    $(document).find(".opm_wrapper").remove();
    $(document).find(".close_opm.modal_hover").remove();
    $(document).off( "change", ".modal.opm .bof_input" );
    $(document).off( "click", ".close_opm" );
    $(document).off( "click", ".modal.opm .buttons .btn-primary" );
  }
