...
window.m2_focus = {
    ...
    switch_type: function( hook, id ){
        ...
        window.m2_focus.log("Chaning type:"+hook)
...
window.m2_focus.ui = {
    ...
    add_cli_htmls: function(){
        ...
        if (!window.m2.isEmbed() && window.app.config.mobile) {
    ...
    setup_player: function( displayData, sourceData ){
        ...
        if ( sourceData?.["type"] ? ( sourceData["type"][1] ? sourceData["type"][1]["preview"] : false ) : false ){
            preview_source = sourceData["type"][1]["preview"]
        }

        var cover = null;
        if (displayData?.cover) {

            const regex = /src(?:set)?="([^"]+)"/g;
            const matches = [...displayData.cover.matchAll(regex)];
    ...
    hook: function () {
        ...
        var hook = window.m2_focus?.sourceData?.data?.ot + "_" + window.m2_focus?.sourceData?.data?.hash;
