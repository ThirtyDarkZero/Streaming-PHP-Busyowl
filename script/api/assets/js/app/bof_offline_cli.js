...
window.bof_offline_cli = {
  ...
  _getWorker:function(){
    ...
    if ( window.config.platform == "web" ){
      ...
      window.bof._loadExtension({
        name: "bof_offline_apk",
        path: "app/bof_offline_apk.js",
      }).done( () => {
        promise.resolve( window.bof_offline_apk );
      } );
    }
  ...
  updateRunning: {
    ...
    dled: function( dl_size, oldPath, newPath, nativeUrl ){
      ...
      window.bof_offline_cli.updateRunning.getItem().then( item => {
        ...
        if ( newPath ){
          console.log( item, oldPath, newPath, nativeUrl );
          if ( item.data?.muse?.source?.type ? ( item.data?.muse?.source?.type[0] == "audio" || item.data?.muse?.source?.type[0] == "video" ) : false ){
            if ( item.data.muse.source.type[1].address == oldPath ){
              item.data.muse.source.type[1].address = newPath
            }
            else if ( item.data.muse.source.type[1].key == oldPath ){
              item.data.muse.source.type[1].key = newPath
            }
            else if ( item.data.muse.source.type[1].slices ? item.data.muse.source.type[1].slices.length : false ){
             for ( var zz=0; zz<item.data.muse.source.type[1].slices.length; zz++ ){
              if ( item.data.muse.source.type[1].slices[zz] == oldPath ){
                item.data.muse.source.type[1].slices[zz] = newPath
              }
             } 
            }
          }
        ...
        if ( oldPath ){
          ...
          item.cached.push( oldPath )
        }

        if ( nativeUrl ){
          item.nativeUrls = item.nativeUrls ? item.nativeUrls : [];
          item.nativeUrls.push( nativeUrl )
        }
